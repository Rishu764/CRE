...
import { Newspaper, FileText, BookOpen, BookMarked, Search, Calendar, Tag, ChevronDown, ChevronUp } from "lucide-react";

const tabs = [
  { id: "policy",    label: "Policy & Institutional",             icon: FileText,   accentColor: "#1b4332" },
  { id: "technical", label: "Technical Research",                 icon: BookOpen,   accentColor: "#2d6a4f" },
  { id: "newspaper", label: "Columnist & Freelance Journalism",   icon: Newspaper,  accentColor: "#b45309" },
  { id: "books",     label: "Books & Bulletins",                  icon: BookMarked, accentColor: "#374151" },
];
...
const technicalPapers = [
  ...
  { no: 75,  ref: "Maithani GP, Bahuguna VK & Sharma DC (1991). Seed germination behavior of Desmodium tilliaefolium — an important shrub species of Himalayas. Indian Forester Vol.117(8): 593-595." },
...
const newspaperArticles = [
  ...
  { no: 173, ref: "Revamp ailing Health Scheme. Pioneer, 28th January 2016." },
...
export default function PublicationsPage() {
  ...
  function handleShowLess() {
    setVisibleCount(ITEMS_PER_PAGE);
    document.getElementById("pub-content")?.scrollIntoView({ behavior: "smooth", block: "start" });
  }
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      {/* ── HERO ── */}
      <section className="relative pt-32 pb-20 bg-[#0a1a14]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <h1 className="text-5xl sm:text-6xl lg:text-7xl font-black text-white mb-5 leading-tight">
            Publications
          </h1>
          <p className="text-base text-white/55 max-w-2xl mb-2 leading-relaxed">
            40 years of writing on environment, climate change, tribal and rural development, and international and national issues.
          </p>
          <p className="text-sm text-white/35 max-w-2xl mb-12 leading-relaxed">
            Articles in national and international publications — Ambio, International Forestry Journals, FAO technical papers — organised by category.
          </p>

          {/* Category stat cards */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              { n: "54",  l: "Policy & Institutional",          id: "policy",    icon: FileText   },
              { n: "74",  l: "Technical Research",              id: "technical", icon: BookOpen   },
              { n: "52",  l: "Columnist & Freelance Journalism", id: "newspaper", icon: Newspaper  },
              { n: "18",  l: "Books & Bulletins",               id: "books",     icon: BookMarked },
            ].map(s => {
              const Icon = s.icon;
              ...
              return (
                ...
                >
                  <Icon className={`w-4 h-4 mb-3 ${isActive ? "text-[#1b4332]" : "text-white/40"}`} />
                  <div className={`text-3xl font-black mb-1 ${isActive ? "text-[#1b4332]" : "text-white"}`}>
                    {s.n}
                  </div>
                  <div className={`text-xs font-medium leading-snug ${isActive ? "text-gray-600" : "text-white/45"}`}>
                    {s.l}
                  </div>
                  {isActive && (
                    <div className="absolute bottom-0 left-4 right-4 h-0.5 bg-emerald-600 rounded-t-full" />
                  )}
                </button>
              );
            })}
      ...
      {/* ── CONTENT ── */}
      <section id="pub-content" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        ...
        <div className="mb-10">
            ...
            {tabs.map(tab => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-xs px-1.5 py-0.5 rounded font-bold ${isActive ? "bg-white/20 text-white" : "bg-gray-100 text-gray-500"}`}>
          ...
          <div className="relative max-w-xl">
            ...
            <input
              ...
              className="w-full pl-10 pr-10 py-3 rounded-lg border border-gray-200 text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 transition-all bg-white"
            />
            {search && (
              <button
        ...
        <div className="flex items-center justify-between mb-6 pb-5 border-b border-gray-200">
          <div className="flex items-center gap-3">
            {(() => { const Icon = activeTabData.icon; return <Icon className="w-5 h-5 text-gray-500" />; })()}
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                ...
                {filtered.length > 0 && ` · showing ${visibleItems.length}`}
              </p>
            </div>
          </div>
        </div>
          ...
          <>
            <div className="space-y-3">
              {visibleItems.map((item, idx) => {
                ...
                const isNew  = idx >= visibleCount - ITEMS_PER_PAGE;
                return (
                  <div
                    key={item.no}
                    className="group relative bg-white rounded-xl p-5 border border-gray-100 hover:border-gray-300 hover:shadow-sm transition-all duration-200"
                    style={isNew ? { animation: "fadeSlideIn 0.3s ease both", animationDelay: `${(idx % ITEMS_PER_PAGE) * 35}ms` } : undefined}
                  >
                    {/* Left accent */}
                    <div
                      className="absolute left-0 top-4 bottom-4 w-0.5 rounded-r-full"
                      style={{ background: activeTabData.accentColor }}
                    />
                    <div className="flex items-start gap-4 ml-3">
                      {/* Number badge */}
                      <div
                        className="shrink-0 w-8 h-8 rounded-lg flex items-center justify-center text-xs font-black text-white mt-0.5"
                        style={{ background: activeTabData.accentColor }}
                      >
                        {item.no}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-gray-700 leading-relaxed">{item.ref}</p>
                      </div>
                    </div>
                  </div>
                );
            ...
            <div className="mt-8 flex flex-col items-center gap-3">
              {hasMore && (
                <button
                  ...
                  className="inline-flex items-center gap-2 px-5 py-2 rounded-lg border border-gray-300 bg-white text-sm font-medium text-gray-600 hover:border-gray-400 hover:text-gray-800 hover:bg-gray-50 transition-all duration-200 shadow-sm"
                >
        ...
        {/* Newspaper ongoing note */}
        {activeTab === "newspaper" && !search && (
          <div className="mt-12 rounded-xl p-5 bg-amber-50 border border-amber-200 flex items-start gap-3">
            <Newspaper className="w-4 h-4 shrink-0 mt-0.5 text-amber-600" />
            <div>
              <p className="text-sm font-semibold text-amber-900 mb-1">Ongoing contributions</p>
